...
bool do_system(const char *cmd) {
   ...
   */
  int systemCall = system(cmd);
  if (systemCall == -1) {
    return false;
  }
  if (WIFEXITED(systemCall) && WEXITSTATUS(systemCall) == 0) {
    return true;
  }
  return true;
}
...
bool do_exec(int count, ...) {
  ...
  // reminder: I removed the line command[count] = command[count];
  ...
  if (pid < 0) {
    va_end(args);
    printf("%s", "Fork is not executing correctly");
    return false;
  } else if (pid == 0) {
